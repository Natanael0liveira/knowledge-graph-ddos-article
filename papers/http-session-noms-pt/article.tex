...
%% Empacotamento de floats: os padroes deixam vaos grandes nas paginas que
%% carregam as duas figuras de largura dupla.
...
\title{Grafos de Conhecimento Centrados na Sessão\\
HTTP para Detecção Explicável e Mitigação\\
de Escopo Derivado de DDoS Distribuído\\
de Camada de Aplicação}

\author{\IEEEauthorblockN{Natanael Oliveira, Arthur Kobielski, Marcos Luna,
Luigi Carro, J\'eferson Campos Nobre e Eder John Scheid}
...
classificador, mesmo com o conjunto completo de atributos de fluxo, enquanto a
representação entre sessões as separa de forma consistente (ROC AUC $0{,}50$
\emph{vs.}\ $0{,}98$; $d = 22{,}4$). Em \emph{datasets} públicos, cujos ataques carregam
...
$39\%$ do tráfego legítimo. Escolher o discriminador por enriquecimento sobre um
perfil
de tráfego normal bloqueia $85$--$90\%$ sem colateral observado. E, avaliado
como detector, esse caminho simbólico dispensa treino de modelo e limiar de escore
aprendido, recuperando de
...
operações por identidade. Na nossa inspeção desses estudos, a sessão é
consistentemente achatada num vetor numérico
...
\textbf{Contribuições.} Este artigo contribui com (i) uma ontologia OWL em que a
sessão HTTP é entidade de primeira classe, relacionada a outras sessões por
\emph{seis sub-propriedades tipadas} ponderadas pelo custo que o atacante teria
para quebrar cada sinal, de modo que a proximidade de rede, trivialmente quebrada
por \emph{botnets} modernas, entra como evidência auxiliar e nunca como
pré-requisito; (ii) um procedimento de decisão em que uma regra SPARQL/SWRL
dispara sobre a massa ponderada de coordenação $\Omega(S)$ de um \emph{cluster}
candidato, de modo que o que se reporta é a derivação que satisfez a regra, e não
um escore de classificador explicado a posteriori; (iii) um escopo de mitigação
derivado desse mesmo grafo e transportado por uma cadeia de evidência em JSON-LD e
STIX~2.1, junto do achado de que a derivação natural, pela propriedade que a
maioria do \emph{cluster} compartilha, é nociva em condições realistas, e de um
critério de enriquecimento sobre um perfil de tráfego normal que a substitui; e
(iv) uma avaliação parametrizada pelo grau de distribuição contra um
\emph{baseline} por sessão deliberadamente \emph{forte}, reportando a fração de
tráfego legítimo que a mitigação resultante bloqueia, métrica auto-reportada por
produtos comerciais de \emph{bot management} mas, até onde sabemos, não usada
sistematicamente na literatura acadêmica de Camada 7.

A Seção~\ref{sec:approach} apresenta o arcabouço, e as
Seções~\ref{sec:method} e~\ref{sec:results} a metodologia e os resultados.
...
\toprule
Trabalho & Ano & Alvo & Unidade de raciocínio & Relação entre sessões & Explicação do veredicto & Escopo de mitigação & Colateral \\
\midrule
Fernandes et al.~\cite{fernandes2015autonomous}   & 2015 & anomalias de fluxo    & estatísticas de fluxo & ---                        & ---                    & ---           & --- \\
Bharathi e Sukanesh~\cite{bharathi2012pca}        & 2012 & DDoS L7               & sessão como vetor     & ---                        & ---                    & ---           & --- \\
Kemp et al.~\cite{kemp2023approach}               & 2023 & DoS L7                & sessão como vetor     & ---                        & ---                    & ---           & --- \\
Sadlek et al.~\cite{sadlek2024device}             & 2024 & dependências de disp. & dispositivo de rede   & implícita (aprendida)      & ---                    & ---           & --- \\
Wang e Stadler~\cite{wang2024intrusion}           & 2024 & intrusão em TI        & \emph{host}           & implícita (aprendida)      & ---                    & ---           & --- \\
KLAGE~\cite{belcastro2025klage}                   & 2026 & ameaças de rede       & nó de rede            & implícita (aprendida)      & pós-hoc (LIME/LLM)     & ---           & --- \\
Gestão de \emph{bots}, indústria~\cite{althouse2023ja4} & ---  & tráfego de \emph{bot} & sessão          & proprietária, similaridade TLS & ---                & não divulgado & autodeclarado \\
\midrule
\textbf{Este trabalho} & --- & DDoS L7 distribuído & sessão de aplicação & explícita (tipada, ponderada) & derivação & derivado & reportado \\
\bottomrule
\end{tabular}

\vspace{2pt}
{\footnotesize Um traço marca uma dimensão fora do escopo declarado do trabalho, não
uma deficiência; \emph{alvo} indica o objetivo que cada trabalho se propõe.}
\end{table*}

A Tabela~\ref{tab:related} posiciona os trabalhos mais próximos nos eixos de que
este artigo trata: a unidade sobre a qual se raciocina, a representação das
relações entre sessões, a forma da explicação, e se um escopo de mitigação e seu
custo colateral decorrem dessa mesma representação.
...
A detecção se organiza historicamente em duas vertentes, sobre um corpo de
...
catalogados, atributos derivados de sessão alimentam o classificador; entre os
setenta e cinco estudos não encontramos nenhum em que a sessão seja mantida como
objeto com identidade, alvo, comportamento e relações tipadas. A redução é uniforme, e é ela que coloca
...
Três componentes padrão sustentam o arcabouço. Um grafo de conhecimento enuncia
fatos como triplas RDF $\langle$sujeito, predicado, objeto$\rangle$, e uma
ontologia OWL~2 declara as classes e propriedades que essas triplas podem usar.
Trabalhamos no perfil OWL~2~RL~\cite{w3c2012owl2profiles}, cujo encadeamento
progressivo polinomial torna a materialização limitada tratável para raciocínio em
tempo de execução onde o OWL~2~DL não é; ela roda uma vez por janela operacional,
e não no caminho por requisição. O SWRL acrescenta regras de Horn, que afirmam uma
tripla quando uma conjunção de premissas vale; o SPARQL consulta o resultado e,
diferentemente do SWRL, agrega sobre conjuntos de solução. A identidade do cliente
vem do JA4~\cite{althouse2023ja4}, um \emph{hash} do \emph{ClientHello} TLS que
caracteriza a pilha TLS do cliente, não o seu endereço, e por isso sobrevive à
rotação de IP.
...
processamento de fluxos RDF. A materialização roda sobre Apache Jena Fuseki com armazenamento TDB2. A Fig.~\ref{fig:pipeline} dispõe as duas camadas e as taxas em
que operam; o algoritmo de construção e o procedimento de decisão par a par de
cada sub-relação \texttt{relatedBy} estão no
...
\caption{O \emph{pipeline} de execução, e a divisão que torna seu custo
suportável. As duas camadas operam em taxas diferentes: a admissão é por
requisição, sobre índices invertidos, enquanto a materialização RDF e a agregação
de $\Omega$ acontecem uma vez por janela, quatro ordens de grandeza adiante por
unidade de trabalho. O teste de enriquecimento lê um perfil de fundo construído
fora de episódios de ataque, de modo que nenhum rótulo entra no caminho de
decisão em momento algum.}
\label{fig:pipeline}
\end{figure*}
...
usuários móveis legítimos atrás de um /24 de CGN, precisa de mais de três vezes
mais pares ligados para atingir o mesmo $\Omega$ que um conjunto ligado por TLS, e
$\tau_{\mathrm{cluster}}$ é calibrado sobre \emph{clusters} legítimos, de modo que
a agregação usual de CGN fica abaixo dele. Isso é proteção explícita contra um
...
materialização par a par da família \texttt{relatedBy} é expressa como regras SWRL
sobre predicados que os extratores de evidência produzem: as relações baseadas em
igualdade são Horn puras, enquanto as numéricas, como distância DTW e similaridade
de cosseno de \emph{payload}, são avaliadas antes e entram na regra como
comparação por \emph{built-in} contra seu limiar. A agregação não é Horn de modo
algum, porque SWRL não soma, então
...
A Listagem~\ref{lst:rules}, no Apêndice~\ref{app:algorithms}, mostra uma regra de
cada tipo.
...
operação normal. Uma propriedade operacionalmente útil decorre disso: quando o adversário se esconde
...
A hipótese central tem duas partes: a representação entre sessões ganha sobre a
detecção por sessão à medida que o \emph{grau de distribuição} $K$ da campanha
cresce, e o caminho simbólico converte essa estrutura numa decisão auditável e de
baixo colateral. A avaliação é, portanto, parametrizada por $K$ e não por nome de
...
Dois parâmetros desse gerador decidem se o cenário se parece com produção, e
ambos são calibrados, não presumidos. O primeiro é a popularidade dos
\emph{fingerprints} benignos, modelada como uma curva de Zipf de expoente
$\alpha$: populações reais são concentradas, já que um navegador atualizado numa
dada plataforma emite essencialmente um JA4~\cite{althouse2023ja4}, ao passo que
sortear os \emph{fingerprints} benignos uniformemente tornaria cada cliente
legítimo quase único. Calibramos essa curva contra $6{,}33$M de requisições TLS registradas num ponto de
...
$38{,}4\%$ das requisições e os dez maiores por $93{,}8\%$. A curva medida é
delimitada por $\alpha = 1{,}5$ e $\alpha = 2{,}0$; adotamos $\alpha = 1{,}5$ como
canônico e varremos a faixa. O segundo é a composição da \emph{botnet}:
\emph{botnets} reais abrangem classes de dispositivo como roteadores, câmeras IP e
DVRs~\cite{antonakakis2017mirai}, e portanto \emph{stacks} TLS, de modo que
distribuímos a campanha uniformemente por $M$ \emph{stacks}, a escolha
conservadora para um dado $M$, com $M = 25$ canônico e varrido porque nenhuma
medição publicada o fixa. Um modo \emph{adversarial} faz ainda a \emph{botnet}
adotar os \emph{fingerprints} benignos mais
comuns em vez dos próprios; isso é operacionalmente viável, e não hipotético: as
...
e \textbf{(d)} a representação completa de \emph{features} entre sessões sobre as três sub-relações observáveis no
tráfego avaliado. Assim,
...
A configuração (d) consome a evidência entre sessões como
...
\textbf{(d) repr. entre sessões}      & \textbf{0,927 [,92--,93]} & \textbf{0,982 [,98--,98]} \\
\midrule
\multicolumn{3}{@{}l}{\emph{(a) e (d) por família de classificador (estimativa pontual)}} \\
...
\vspace{2pt}
{\footnotesize A configuração (d) alcança a evidência entre sessões por \emph{features}, e não
por SPARQL, de modo que esta tabela mede a representação; o caminho simbólico é
avaliado à parte na Tabela~\ref{tab:symbolic}.
$^{\dagger}$ abaixo do acaso: o \emph{perceptron} inverte, tendo
...
por construção, amostrada das distribuições benignas. A representação entre sessões (d)
alcança $0{,}93$--$0{,}98$. A configuração (c), restrita à proximidade de rede, é
fraca ($0{,}50$--$0{,}66$) porque a \emph{botnet} está dispersa.
...
classificador, sem treino e sem limiar de classificação aprendido, contra o modelo
aprendido forçado ao mesmo
ponto de operação --- com exceção da linha adversarial, discutida no texto.
$K = 1000$, $n = 15$ sementes.}
\label{tab:symbolic}
\scriptsize
...
$85{,}0\%$ contra $17{,}6\%$ em cem \emph{stacks}, e a regra mantém a vantagem
mesmo contra um modelo autorizado a gastar falsos positivos: a $1\%$ de FPR ele
chega a apenas $66{,}8\%$ e $44{,}5\%$. A linha adversarial \emph{não} é
comparação equivalente e não afirmamos nada a partir dela --- ali a própria regra
incorre em $3{,}78\%$ de falso positivo, e o modelo chega a $23{,}4\%$ quando lhe permitem $1\%$, de modo
que a ordem num ponto de operação equiparado fica em aberto.
...
fundo explícito, e é por isso que sua taxa de falso positivo é nula nesses
cenários como consequência do critério de enriquecimento, e não de um limiar de
escore ajustado. Isso não é garantia incondicional.
...
representação entre sessões sobre ele é marginal neste ataque convencional.}
\label{tab:real}
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{@{}lcccc@{}}
...
Neste ataque real a coordenação entre sessões não é necessária: um classificador
por sessão competente o separa, porque o ataque nunca foi calibrado para mimetizar
o fluxo benigno. A comparação com o KLAGE é favorável mas \emph{não controlada}: nosso
$0{,}911$ supera o $0{,}841$ publicado, mas nosso próprio \emph{baseline} forte
também o supera com $0{,}900$, de modo que a margem não pode ser atribuída
especificamente à representação entre sessões. Uma reexecução controlada não
...
A necessidade da evidência entre sessões aparece apenas no regime furtivo
distribuído, que os \emph{datasets} públicos de DDoS, restritos a ataques com
assinatura de fluxo evidente, não contêm (Fig.~\ref{fig:regime},
Apêndice~\ref{app:limitations}). Incondicionalmente sobre o estado da arte em
nível de nó, a abordagem acrescenta um veredicto simbólico auditável e uma medida
de dano colateral.
...
\emph{stacks} ela não bloqueia atacante algum e atinge $39\%$ do tráfego
legítimo. O enriquecimento é invariante à fragmentação e, sob adoção adversarial
...
atacantes bloqueadas sem colateral observado, contra $100\%$ para um
...
\textbf{sem colateral observado} nas $n = 15$ campanhas, e reproduzindo a regra por
frequência onde esta
...
$39{,}0\%$ da regra por frequência. A vantagem de seletividade é em larga medida
...
otimização. Isso implica também um \emph{fail-safe}. Quando o perfil falha numa
verificação de frescor ou de desvio, a mitigação de escopo derivado deve ser
suprimida e o veredicto emitido apenas como evidência --- a mesma abstenção que o
teste de enriquecimento já pratica quando nada está enriquecido.

\subsection{Custo e a Escolha de $W$}
\label{sec:latency}

O Apêndice~\ref{app:cost} mede as duas camadas do \emph{pipeline} em função da
ocupação da janela. Dois resultados são operacionalmente relevantes: a admissão
custa constantes $14$--$20$~ns por par candidato, sustentando cerca de $84\,000$
admissões de sessão por segundo em um núcleo com $|S_W| = 1\,000$; e a detecção é
plana numa faixa de $30\times$ em $W$, enquanto a ocupação do \emph{cluster}, e com
ela o termo quadrático de pares que $\Omega(S)$ implica, cresce $6{,}6\times$. Isso
favorece escolher $W$ tão pequeno quanto o tráfego permitir.
...
\textbf{(d)$-$(a)} com $p_{\mathrm{Bonf}} = 7{,}5\times 10^{-9}$ e $d = 22{,}4$. A
correlação rank-biserial pareada é $+1{,}00$ nos dois contrastes: (d) vence nas 30
execuções pareadas. Tamanhos de efeito dessa magnitude refletem a natureza controlada do tráfego
...
Cinco limitações principais delimitam nossas afirmações; o
Apêndice~\ref{app:limitations} lista outras.
...
sessão forte já alcança $\mathrm{AUC} \geq 0{,}98$, deixando à representação
entre sessões apenas um ganho marginal. A vantagem é decisiva apenas quando a campanha é
...
enfraquecido, já que quatro famílias de classificador com todos os atributos de fluxo
...
configurada, o particionamento por \emph{endpoint} e uma janela limitada devem
portanto manter o termo quadrático fora do caminho por requisição, embora
confirmar isso em escala de CDN siga como trabalho futuro.

\textbf{Escopo da ablação.} A configuração (d) alcança a
...
representar estrutura entre sessões, e não a expressá-la em OWL. O formalismo não
tem por objetivo elevar a AUC; ele fornece um vocabulário tipado e inspecionável
em que veredicto, evidência e escopo de mitigação formam uma única derivação,
exportável para STIX sem etapa de tradução. A camada simbólica
...
afirmações devem, portanto, ser interpretadas separadamente.
...
quatro famílias de classificador, mesmo com o conjunto completo de atributos de
fluxo,
...
um classificador por sessão forte já basta. Do lado da mitigação, o estudo também
expõe um resultado negativo operacionalmente importante: derivar o escopo pela
propriedade que a maioria do \emph{cluster} compartilha seleciona um
\emph{fingerprint} legítimo assim que a \emph{botnet} abrange vários
\emph{stacks} TLS, bloqueando usuários em vez de atacantes. O enriquecimento sobre
um perfil de tráfego normal restaura $85$--$90\%$ de cobertura sem colateral
observado. E, medido como detector, esse caminho simbólico recupera $90\%$ de uma
...
evidência mais clara que temos do valor operacional do caminho simbólico. Direções imediatas são os ataques
...
\section{Construção em Tempo de Execução e Instanciação das Sub-Relações}
...
Para cada par $(s_a,s_b)$ ativo em $W$, os procedimentos a seguir decidem se a
aresta é instanciada.
...
\subsection*{Realismo do Gerador}

Duas ressalvas se aplicam à medição em produção da popularidade dos
\emph{fingerprints} benignos. O \emph{log} não carrega identificador de conexão,
então a ponderação é por requisição e não por sessão, o que superpondera clientes
pesados e, se algo, \emph{superestima} a concentração; e a amostra é de um ponto
de presença numa janela. O \emph{testbed}
CICIDS2017~\cite{sharafaldin2018cicids} é ainda mais concentrado ($52{,}7\%$ de
cabeça), de modo que a forma medida em laboratório é apenas indicativa. A curva
medida também não é exatamente Zipf: sua cabeça corresponde a $\alpha = 1{,}5$
($38{,}9\%$ contra $38{,}4\%$) enquanto seu top dez corresponde a
$\alpha = 2{,}0$ ($94{,}2\%$ contra $93{,}8\%$), e é por isso que a varredura a
delimita em vez de ajustar um expoente único.
...
Apêndice~\ref{app:cost} são um limite superior, pela razão dada lá; e escalar
para tráfego em escala de CDN requer o particionamento horizontal da
Seção~\ref{sec:discussion}, que não implementamos.
...
\textbf{Cobertura de sub-relações.} O $\Omega(S)$ avaliado instancia três das seis
sub-relações --- \emph{fingerprint} TLS, convergência de \emph{endpoint},
proximidade de rede ---, as que o gerador e as capturas públicas expõem.
Reaproveitamento de identidade, padrão temporal e assinatura de \emph{payload}
estão no Apêndice~\ref{app:algorithms} mas não são exercitados, então os pesos da
Fig.~\ref{fig:ontology} só estão validados para essas três.
...
tráfego de produção anonimizado ficam como trabalho futuro. A lacuna persiste em
\emph{benchmarks} recentes: o BCCC-cPacket-Cloud-DDoS-2024~\cite{shafi2024bccc}
aponta a representação insuficiente de ataques de camada de aplicação como
deficiência de \emph{dataset}, mas seus 17 cenários de DDoS são todos baseados em
TCP. Entre os \emph{benchmarks} examinados não encontramos, portanto, captura
independente do regime furtivo de Camada 7 estudado aqui.

\textbf{Validação da explicação.} As cadeias de evidência passam nas verificações
automatizadas de completude e acionabilidade da Seção~\ref{sec:method} ($7/7$).
Não conduzimos estudo com usuários, de modo que se elas encurtam o tempo até a
decisão de um analista permanece não testado.

\begin{figure}[tb]
\centering
\includegraphics[width=\columnwidth]{figures/fig4_regime.png}
\caption{Onde a evidência entre sessões é \emph{necessária}. Em ataques reais
convencionais um classificador por sessão forte já satura; só no regime furtivo
distribuído a detecção por sessão colapsa para o acaso enquanto a representação
entre sessões segue discriminativa.}
\label{fig:regime}
\end{figure}
...
admissão implementa o esquema de índices invertidos da
Seção~\ref{sec:pipeline}, e a camada simbólica roda sobre \texttt{rdflib} em
memória, e não sobre o \emph{backend} Jena/TDB2 usado no \emph{pipeline} de
\emph{datasets}, de modo que seus números são um limite
superior.
...
arestas por admissão) a constantes $14$--$20$~ns por par.
...
o custo permanece entre $180$ e $210\,\mu$s por aresta RDF materializada em
...
$1800$~s: o atributo discriminativo é a \emph{fração} de um \emph{cluster} que
compartilha um mesmo JA4, que não muda conforme o \emph{cluster} cresce, ao passo
que o tamanho sozinho carrega pouco --- daí (c) em $0{,}664$ o tempo todo. Dois
limites: os cenários têm um único \emph{endpoint} alvo, então $W$ é o único botão
de agrupamento e uma implantação multi-\emph{endpoint} pode se comportar de outra
forma; e $W$ precisa ser longo o bastante para um \emph{cluster} se formar, o que
em $W = 60$~s já significa $\approx 132$ sessões.
...
\subsection*{Particionamento em Escala de CDN}

O desenho que o particionamento por \emph{endpoint} sugere, não implementado
aqui, é de dois níveis: trabalhadores particionados por \emph{endpoint} mais um
\emph{sketch} global sobre identificadores de peso alto, para recuperar campanhas
que se espalham por vários \emph{endpoints}. Quantificar o \emph{recall} perdido
pelo primeiro nível e recuperado pelo segundo é pré-requisito para implantação em
escala de CDN. Com o modelo de custo acima, o argumento de escalabilidade é que o
particionamento por \emph{endpoint} é exato e não exige coordenação, que um $W$
pequeno corta o termo quadrático de pares sem custo em detecção, e que a admissão
custa microssegundos; confirmar isso em escala de CDN permanece trabalho futuro.
...
gerador sintético calibrado, com os três parâmetros de realismo da
Seção~\ref{sec:method} e as configurações de cenário canônico e adversarial; a ablação, os \emph{baselines} acadêmicos e as quatro
famílias de classificador; o executor estatístico; a comparação contra o estado da arte
...
As duas derivações de escopo são distribuídas lado a lado: a regra por frequência
que falha e a regra por enriquecimento que a substitui, de modo que o resultado
negativo da Seção~\ref{sec:collateral} seja reprodutível, e não apenas afirmado. O perfil de fundo de que o teste de enriquecimento precisa é produzido
