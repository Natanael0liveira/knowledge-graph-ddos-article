...
%% O IEEEtran usa URW Courier (pcr) para \texttt. Cai para Computer Modern
%% typewriter quando essas métricas não existem localmente; no Overleaf e em
%% instalações completas a fonte padrão do IEEE é mantida.
...
em tempo de execução. Três propriedades, contudo, permanecem em aberto. Sua
...
A detecção se organiza historicamente em três famílias. O perfilamento estatístico
...
validado em tempo real e que a saída é binária, sem explicação ontológica. A
terceira família consolida o domínio em
levantamentos~\cite{tripathi2021application,cambiaso2013slowdos}. O denominador
...
A A Fig.~\ref{fig:rules}, no Apêndice~\ref{app:algorithms}, mostra uma regra de
cada tipo.
...
espera-se que detectores volumétricos clássicos bastem; ele é degenerado para AUC
...
todas produzem um filtro diretamente aplicável (Fig.~\ref{fig:chain}, Apêndice~\ref{app:cost}). Um estudo
...
monolítica ou pouco fragmentada, o modelo aprendido empata ou supera a regra a
...
Testes pareados de Wilcoxon sobre as $n = 30$ execuções, com correção de Bonferroni
e $d$ de Cohen. No regime distribuído ($K = 1000$), \textbf{(d)$-$(c)} é
...
Cinco limitações delimitam a afirmação; outras estão no
...
sessões uma margem marginal. A vantagem é decisiva apenas quando a campanha é
...
mostra o mecanismo sob a hipótese de mimetismo por sessão perfeito, e não prova
...
Seção~\ref{sec:latency}, as três medições compõem um único argumento de
...
ROC AUC; $d = 13{,}5$); em ataques reais convencionais o ganho é marginal, porque
...
\textbf{Custo de manutenção do grafo.} Os números da camada simbólica da
Seção~\ref{sec:latency} vêm de uma implementação de referência em memória com
...
\textbf{Cobertura de protocolo.} A ontologia atual cobre a família Slowloris e seus
descendentes diretos sobre HTTP/1.1. Ataques sobre HTTP/2 e outros protocolos de
...
tabela e figura deste artigo é regenerada por um único comando por etapa. A
