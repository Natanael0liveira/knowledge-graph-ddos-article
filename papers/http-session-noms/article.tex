...
endpoint, exhibited behaviour and applicable mitigation, and replaces the flat
...
behavioural matrix. The latter is among the first formulations to put the session
at the centre, though still as a feature vector. Supervised learning over session
...
behaviour and typed relations. The reduction is uniform, and it is precisely what places cross-session
...
entity with typed primary relations to identity, origin, endpoint and behaviour;
...
Fig.~\ref{fig:rules} in Appendix~\ref{app:algorithms} shows one rule of each
kind.
...
filter (Fig.~\ref{fig:chain}, Appendix~\ref{app:cost}). A human-analyst study remains future work.
