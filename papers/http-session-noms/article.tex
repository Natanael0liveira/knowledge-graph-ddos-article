...
from it --- there the rule itself leaks $3.78\%$, and the model reaches $23.4\%$
...
which is why its false-positive rate is zero wherever an enriched discriminator
exists --- zero by construction rather than by tuning, but not unconditionally.
...
Five limitations bound the claim, interleaved below with two paragraphs that
place the framework operationally; further ones are listed in
