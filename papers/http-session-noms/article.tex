...
which is why the sweep brackets it rather than fitting a single exponent.
...
deployment. Taken together with the cost model above, these observations compose
one scalability argument: endpoint sharding is exact and needs no coordination, a
small $W$ costs no detection while cutting the quadratic pair term, and admission
is microseconds. Endpoint partitioning and a bounded window should therefore keep
the quadratic component off the per-request path, though confirming that at CDN
