...
weighted \texttt{relatedTo} family.} An OWL ontology replaces the flat
\texttt{relatedTo} link with \emph{six typed sub-properties} weighted by the
attacker's cost of breaking each signal; the evaluation exercises the three that
the available traffic exposes. Network proximity, trivially broken by modern
botnets, therefore enters as auxiliary evidence and never as a prerequisite.

\item \textbf{Symbolic reasoning over their weighted sum.} A SPARQL/SWRL rule
fires on the weighted coordination mass $\Omega(S)$ of a candidate cluster, so
the verdict is the derivation that satisfied the rule rather than a classifier
output interpreted after the fact.

\item \textbf{An evidence chain coupled to automatic mitigation scoping.} Each
verdict carries a JSON-LD and STIX~2.1 chain, and the mitigation scope is derived
from the same graph. We show that the natural way to derive it, the property most
of the cluster shares, is actively harmful under realistic conditions, and
replace it with a criterion based on enrichment over a profile of normal traffic.

\item \textbf{An evaluation parameterized by the degree of distribution, with a
collateral-damage metric.} Against a deliberately \emph{strong} per-session
baseline, the ablation isolates the gain of cross-session representation. We also
report the fraction of legitimate traffic hit by the resulting mitigation, a
metric self-reported by commercial bot-management products but, to our knowledge,
not used systematically in the academic Layer-7 literature.
\end{enumerate}
...
\caption{Runtime pipeline. Admission runs per request over inverted indexes;
RDF materialization and the $\Omega$ aggregation run once per window. The
enrichment test reads a background profile built outside attack episodes.}
\label{fig:pipeline}
\end{figure*}
...
\begin{table}[t]
\centering
\caption{The SPARQL/enrichment path evaluated end to end as a detector: the
sessions matching the derived scope \emph{are} the flagged sessions. Compared
against the learned model forced onto the same operating point, except in the
adversarial row (see text). $K = 1000$, $n = 15$ seeds.}
\label{tab:symbolic}
\scriptsize
...
\caption{Attack blocked and legitimate traffic hit, choosing the discriminator by
frequency versus by enrichment, against botnet fragmentation $M$ under realistic
benign fingerprint concentration ($\alpha = 1.5$). $n = 15$ seeds.}
...
\caption{Cost of the two layers against window occupancy, log--log. Reference
slopes are anchored on the first point of each series.}
\label{fig:latency}
\end{figure}
...
abridged). Every term of $\Omega(S) = 10.8$ is attributable to a sub-relation, a
pair count and a weight; the last field is the filter applied.}
\label{lst:chain}
\end{listing}
