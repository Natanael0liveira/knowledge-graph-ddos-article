...
baseline already suffices, a boundary we report explicitly. Scoping the
mitigation by the property most of the cluster shares is harmful: it selects a
\emph{legitimate} fingerprint once the botnet spans several TLS stacks, blocking
$0\%$ of the attack and $39\%$ of legitimate traffic, whereas choosing the
discriminator by enrichment blocks $85$--$90\%$ with no collateral observed. And
measured as a detector, the symbolic path needs no model training and no learned
score threshold, yet recovers two to five times more of a fragmented campaign
than the learned model does at the same zero-false-positive operating point.
\end{abstract}
...
\textbf{Contributions.} Each contribution below names the section that delivers
it and the evidence that substantiates it.

\begin{enumerate}
\item \textbf{The session as a first-class ontological entity.} An OWL ontology
relates sessions through \emph{six typed sub-properties} weighted by the
attacker's cost of breaking each signal, so that network proximity, trivially
broken by modern botnets, enters as auxiliary evidence and never as a
prerequisite (Section~\ref{sec:approach}).

\item \textbf{The verdict as a derivation.} A SPARQL/SWRL rule fires on the
weighted coordination mass $\Omega(S)$ of a candidate cluster, so what is
reported is the derivation that satisfied the rule, not a classifier score
explained after the fact (Section~\ref{sec:approach}).

\item \textbf{Mitigation scope derived from the same graph.} Each verdict
carries a JSON-LD and STIX~2.1 evidence chain. We show that the natural
derivation, the property most of the cluster shares, is actively harmful under
realistic conditions, and replace it with an enrichment criterion over a profile
of normal traffic (Section~\ref{sec:approach}).

\item \textbf{An evaluation parameterized by the degree of distribution.}
Against a deliberately \emph{strong} per-session baseline, an ablation isolates
the gain of cross-session representation and reports the fraction of legitimate
traffic the resulting mitigation blocks, a metric self-reported by commercial
bot-management products but not, to our knowledge, used systematically in the
academic Layer-7 literature (Sections~\ref{sec:method} and~\ref{sec:results}).
\end{enumerate}
...
Table~\ref{tab:related} positions the closest work on the axes this paper
turns on: the unit over which reasoning happens, whether relations between
sessions are represented at all, whether the verdict carries an explanation,
whether a mitigation scope follows from the same representation, and whether
the legitimate traffic that scope blocks is reported.
...
Three standard components carry the framework. A knowledge graph states facts as
RDF triples $\langle$subject, predicate, object$\rangle$, and an OWL~2 ontology
declares the classes and properties those triples may use. We work in the
OWL~2~RL profile~\cite{w3c2012owl2profiles}, whose rule-based entailment admits
forward-chaining materialization in polynomial time and is therefore admissible
at line rate, where full OWL~2~DL reasoning is not. SWRL extends the ontology
with Horn rules, which materialize a new triple when a conjunction of premises
holds, and SPARQL queries the resulting graph and, unlike SWRL, aggregates over
solution sets. Client identity comes from JA4~\cite{althouse2023ja4}, a hash of
the TLS \emph{ClientHello} that characterizes a client's TLS stack rather than
its address and so persists across IP rotation.
...
we sweep it rather than choose a single value. Finally, an \emph{adversarial}
mode lets the botnet adopt the most common benign fingerprints instead of its
own; this is operationally feasible rather than hypothetical: browser-impersonation
tooling ships ready ClientHello templates for specific Chrome and Safari versions.
...
The design it suggests, unimplemented, is two-tiered:
