...
\begin{abstract}
%% 200-300 palavras
\textbf{Campanhas coordenadas} de DDoS de Camada 7 sobre HTTP --- a família de \emph{Slow HTTP DDoS distribuído} (Slowloris, slow body, slow read, HULK, GoldenEye, e variantes modernas em HTTP/2 como Rapid Reset e CONTINUATION Flood), \emph{credential stuffing}, e abuso de API por múltiplas identidades --- imitam o comportamento de usuários legítimos a baixas taxas por sessão e passam por defesas baseadas em assinaturas, limiares estatísticos e quotas por IP, sessão ou \emph{token}. O sinal de ataque não está em nenhuma sessão isolada; está na correlação estrutural entre sessões que compartilham alvo, \emph{fingerprint} TLS, prefixo de IP ou identidade reaproveitada. A literatura de detecção reduz a sessão a um vetor numérico antes do classificador, descartando exatamente a estrutura que define a campanha; o detector não consegue articular \emph{por que} uma sessão é suspeita, e perde a estrutura \emph{cross-session}. Grafos de conhecimento em cibersegurança, por sua vez, têm sido construídos estaticamente a partir de texto de inteligência de ameaças, não como mecanismo de detecção em tempo de execução. Este artigo propõe um arcabouço de grafos de conhecimento que modela a sessão HTTP como objeto semântico de primeira classe, ligando-a por relações tipadas à identidade do cliente, ao \emph{endpoint} alvo, a sinais comportamentais e a mitigações aplicáveis. Apresentamos uma ontologia OWL com três especializações de ataque coordenado, alinhada a STIX 2.1 e MITRE ATT\&CK. Requisições são elevadas em tempo de execução a um grafo cujas regras semânticas produzem veredictos com cadeias de evidência atravessando múltiplas sessões, e cujo discriminador do \emph{cluster} detectado é usado para derivar automaticamente o escopo da mitigação aplicada, preservando tráfego legítimo. A avaliação empírica é parametrizada pelo \emph{grau de distribuição} da campanha e isola, por ablação, o ganho do raciocínio em nível de sessão em relação a detectores que consomem as mesmas \emph{features} de sessão na forma agregada, sob uma métrica adicional de fração de tráfego legítimo afetado pela mitigação resultante. A implementação é disponibilizada como código aberto.
...
A categoria central nesse novo regime é o \textbf{Slow HTTP DDoS distribuído}: a família que inclui Slowloris e seus descendentes diretos (slow body, slow read), suas variantes de alta taxa (HULK, GoldenEye), e os ataques modernos sobre HTTP/2 (Rapid Reset, CONTINUATION Flood, MadeYouReset). O denominador comum é \emph{sub-limiar por sessão}: cada origem contribui pouco, o conjunto exaure o servidor. Ao lado dessa família, dois outros vetores compõem o conjunto de ataques coordenados de Camada 7 que motivam este trabalho: \emph{credential stuffing} contra \texttt{/api/auth/login}, e abuso de API distribuído por uma frota de \emph{tokens}. Em todos, vista isoladamente, cada requisição é indistinguível de uma legítima: mesma estrutura, mesmos cabeçalhos, muitas vezes até mesma frequência. O sinal de ataque mora no padrão de uso da sessão e, mais ainda, no padrão estrutural entre sessões: quantas falhas de login em quantas identidades, qual a entropia das rotas visitadas, quão consistente é o \emph{fingerprint} do cliente, quão repetitivo é o ritmo de chamadas à API~\cite{kemp2023approach,bharathi2012pca,fernandes2015autonomous}.
...
\begin{description}
    \item[\textbf{QP1:}] Como uma ontologia OWL pode modelar a sessão HTTP, junto a endpoint, identidade e comportamento, com fidelidade suficiente para sustentar raciocínio sobre campanhas coordenadas de Camada 7 (Slow HTTP DDoS distribuído, \emph{credential stuffing}, abuso de API por múltiplas identidades) em vez de meramente agregar \emph{features} ao nível de sessão?
...
\begin{enumerate}
    \item Uma ontologia OWL para detecção de campanhas coordenadas de DDoS de Camada 7 sobre HTTP que modela a sessão HTTP como classe semântica de primeira classe (\texttt{ApplicationSession}), com relações tipadas para identidade do cliente (\texttt{hasIdentity}), endpoint alvo (\texttt{targets}), comportamento exibido (\texttt{exhibitsBehavior}), correlação \emph{cross-session} (\texttt{relatedTo}) e mitigação aplicável (\texttt{mitigatedBy}), e três especializações concretas de ataque coordenado (Slow HTTP DDoS distribuído --- englobando Slowloris, slow body, slow read, HULK, GoldenEye e variantes modernas em HTTP/2 ---, \emph{credential stuffing}, abuso de API por múltiplas identidades) alinhadas a STIX 2.1 e MITRE ATT\&CK.
